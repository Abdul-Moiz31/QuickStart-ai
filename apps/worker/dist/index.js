...
import { chunkText, createEmbeddingsClient, storeChunkEmbeddings, } from "@quickstart-ai/rag";
...
async function processIngest(documentId, projectId) {
    ...
    const created = await prisma.$transaction(chunks.map((content, chunkIndex) => prisma.knowledgeChunk.create({
        data: {
            documentId,
            projectId,
            content,
    ...
    await storeChunkEmbeddings(created.map((c) => c.id), vectors);
    await prisma.knowledgeDocument.update({
        where: { id: documentId },
        data: { status: "READY" },
    });
...
async function main() {
    ...
    const worker = new Worker(QUEUE_NAMES.INGEST, async (job) => {
    ...
    console.log("QuickStart worker listening on queues:", QUEUE_NAMES.INGEST, QUEUE_NAMES.EVAL, QUEUE_NAMES.EVENTS, QUEUE_NAMES.EVENTS_RETRY);
