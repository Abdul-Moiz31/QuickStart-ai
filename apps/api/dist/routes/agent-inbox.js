...
import { inboxChannel, publishInboxEvent, publishSessionEvent, sessionChannel, } from "../realtime.js";
...
export async function agentInboxRoutes(app) {
    ...
    app.patch("/api/v1/agent/sessions/:sessionId/takeover", async (req) => {
        ...
        const now = new Date();
        const claimed = await Session.findOneAndUpdate({ _id: session._id, humanActive: { $ne: true } }, {
            $set: {
                humanActive: true,
                humanPending: false,
                agentId: req.user.id,
        ...
        if (!claimed)
            throw new ConflictError("This conversation was already taken by another agent");
