...
import { createChatClient, createEmbeddingsClient, getCachedAnswer, pushSessionMemory, runAgenticRag, setCachedAnswer, } from "@quickstart-ai/rag";
import { AppError, computeEvalReadiness, EVAL_PASS_THRESHOLDS, MIN_KNOWLEDGE_QA, NotFoundError, parseKnowledgeQa, QUEUE_NAMES, } from "@quickstart-ai/shared";
...
import { collectGapCandidates, GAP_CACHE_PERIODS, gapCacheKey, periodToSince, } from "../knowledge-gaps.js";
import { groupAndResolve } from "../knowledge-gap-grouping.js";
import { analyticsCacheKey, ANALYTICS_PERIODS, computeProjectAnalytics, } from "../analytics.js";
...
const GAP_PERIODS = [...GAP_CACHE_PERIODS];
async function loadKnowledgeQaCases(projectId) {
    const docs = await prisma.knowledgeDocument.findMany({
        where: { projectId, status: "READY" },
        select: { id: true, rawContent: true },
    });
    const pairs = docs.flatMap((d) => parseKnowledgeQa(d.rawContent ?? ""));
    const cases = pairs.slice(0, MAX_EVAL_CASES).map((p, i) => ({
        id: `qa-${i + 1}`,
        question: p.question,
        expected: p.answer,
    }));
...
async function enqueueEval(runId, projectId) {
    const queue = getEvalQueue();
    try {
        await queue.add("run-eval", { runId, projectId }, {
            removeOnComplete: 50,
            removeOnFail: 20,
            attempts: 1,
            jobId: runId,
...
function isVisitorSession(s) {
    const email = String(s.visitorEmail ?? "");
    const name = String(s.visitorName ?? "");
    const meta = (s.metadata ?? {});
    if (meta.playground)
        return false;
    if (email.includes("+admin-test@"))
        return false;
    if (name.includes("(admin test)") || name.includes("(admin)"))
        return false;
    return true;
}
...
export async function dashboardRoutes(app) {
    app.get("/api/v1/projects/:id/sessions", async (req) => {
        ...
        const sessions = await Session.find({ projectId: id })
            ...
            .limit(200)
            .lean();
        const visitorSessions = sessions.filter(isVisitorSession);
        return {
            success: true,
    ...
    app.get("/api/v1/projects/:id/sessions/:sessionId", async (req) => {
        ...
        return { success: true, session: formatSessionDetail(session) };
    });
    ...
    app.post("/api/v1/projects/:id/playground/message", async (req) => {
        ...
        try {
            ...
            const message = err instanceof Error ? err.message : "Chat model temporarily unavailable";
            console.error("[playground] RAG failed", err);
            const fallback = "I'm having trouble reaching the AI provider right now (rate limit or outage). Please try again in a few seconds.";
            session.messages.push({
                role: "assistant",
                content: fallback,
    ...
    app.get("/api/v1/projects/:id/analytics", async (req) => {
        ...
        const period = ANALYTICS_PERIODS.includes(rawPeriod)
            ? rawPeriod
            : "30d";
        const project = await prisma.project.findFirst({
            where: { id, ownerId: req.user.id },
    ...
    app.get("/api/v1/projects/:id/eval/status", async (req) => {
        ...
        const lastMetrics = (lastRun?.metrics ?? null);
    ...
    app.post("/api/v1/projects/:id/eval/run", async (req) => {
        ...
        if (pairs.length < MIN_KNOWLEDGE_QA) {
            throw new AppError(`Add at least ${MIN_KNOWLEDGE_QA} knowledge Q&A pairs (Q: / A:) before running eval. You have ${pairs.length}.`, 400);
        }
