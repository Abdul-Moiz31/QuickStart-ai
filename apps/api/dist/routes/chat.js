...
import { createChatClient, createEmbeddingsClient, getCachedAnswer, pushSessionMemory, runAgenticRag, runAgenticRagStream, setCachedAnswer, } from "@quickstart-ai/rag";
import { collectAndEmitChatEvents } from "@quickstart-ai/events";
import { AppError, BUILTIN_EVENT_TYPES, chatMessageSchema, createSessionSchema, NotFoundError, PLAN_LIMITS, } from "@quickstart-ai/shared";
...
function streamError(reply, message) {
    writeSseEvent(reply, { type: "error", message });
    endSse(reply);
}
const EMPTY_ANSWER_FALLBACK = "Sorry, I couldn't generate a response right now. Please try again.";
async function resolveEmptyStreamAnswer(accumulated, preamble, chat, modelChainRotate) {
    if (accumulated.trim())
        return accumulated;
...
function didEscalate(events) {
    return events.some((e) => e.type === BUILTIN_EVENT_TYPES.HUMAN_HANDOFF);
}
async function isStillBotControlled(Session, sessionId) {
    const fresh = await Session.findById(sessionId).select("humanActive").lean();
    return !fresh?.humanActive;
}
async function markSessionEscalated(Session, sessionId, projectId, visitorName, triggerMessage) {
    const now = new Date();
    await Session.updateOne({ _id: sessionId, humanActive: { $ne: true } }, { $set: { humanPending: true, escalatedAt: now } });
...
export async function chatRoutes(app) {
    ...
    app.post("/api/v1/chat/sessions/:sessionId/request-handoff", async (req) => {
        ...
        if (!session.humanPending) {
            await markSessionEscalated(Session, sessionId, session.projectId, session.visitorName, "Visitor requested support");
        }
    ...
    app.post("/api/v1/chat/message", async (req, reply) => {
        ...
        if (!project)
            throw new NotFoundError("Project not found");
        const planKey = (project.plan ?? "free");
        ...
        if (todayCount >= dailyLimit) {
            throw new AppError(`Daily limit of ${dailyLimit} messages reached for your ${planKey} plan. Upgrade or wait until tomorrow.`, 429, "PLAN_LIMIT_EXCEEDED");
        }
        await connectMongo();
        const Session = getChatSessionModel();
        ...
        if (body.stream) {
            ...
            accumulatedAnswer = await resolveEmptyStreamAnswer(accumulatedAnswer, preamble, chat);
