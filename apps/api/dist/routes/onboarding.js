...
import { ONBOARDING_MODEL_CHAIN, chatWithModelFallback, fetchWebsiteSummary, } from "@quickstart-ai/rag";
import { AppError, classifyKnowledgeDoc, extractQaFromOnboardingDoc, onboardingBusinessSchema, onboardingCompleteSchema, parseBusinessProfileExtras, rebuildOnboardingDocContent, updateBusinessProfileSchema, } from "@quickstart-ai/shared";
import { requireAuth } from "../auth.js";
import { generateClientId, generateClientSecret, hashSecret, } from "../credentials.js";
...
Generate ${MAX_QUESTIONS} onboarding questions tailored to this business.`,
        ...
        ], ONBOARDING_MODEL_CHAIN, { temperature: 0.4, maxTokens: 1200 });
...
export async function onboardingRoutes(app) {
    ...
    app.patch("/api/v1/onboarding/business", async (req) => {
        ...
        if (!merged.businessName || !merged.businessIndustry || !merged.businessDescription) {
            throw new AppError("businessName, businessIndustry, and businessDescription must be set before partial updates", 400);
        }
...
Answer only from the business knowledge provided. Be clear, helpful, and brief (widget-sized replies).`;
        ...
        if (onboardingDoc && project) {
            ...
            try {
                ...
                await queue.add("ingest-document", { documentId: onboardingDoc.id, projectId: project.id }, { removeOnComplete: 100, removeOnFail: 50, attempts: 3 });
                await queue.close();
            }
            catch {
                // doc stays PENDING
...
Answer only from the business knowledge provided. Be clear, helpful, and brief (widget-sized replies).`;
        ...
        try {
            const queue = getIngestQueue();
            await queue.add("ingest-document", { documentId: doc.id, projectId: project.id }, { removeOnComplete: 100, removeOnFail: 50, attempts: 3 });
            await queue.close();
        }
        catch {
