import { vectorSearch } from "@quickstart-ai/rag";
 ...
 */
function clusterByEmbedding(candidates, vectors) {
    const groups = [];
...
function representativeQuestion(members) {
...
export async function groupAndResolve(opts) {
    ...
    for (const cluster of clusters) {
        ...
        groups.push({
            ...
            lastAskedAt: members.reduce((latest, m) => (m.askedAt > latest ? m.askedAt : latest), members[0].askedAt),
...
function groupByExactText(candidates) {
...
//# sourceMappingURL=knowledge-gap-grouping.js.map