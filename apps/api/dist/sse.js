...
export function beginSseReply(req, reply) {
    reply.hijack();
...
export async function streamChannels(opts) {
    ...
    if (initialEvent)
        write(initialEvent);
    const handles = await Promise.all(channels.map((channel) => subscribeChannel(channel, write)));
    if (closed) {
