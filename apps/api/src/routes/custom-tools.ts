...
export async function customToolsRoutes(app: FastifyInstance) {
  ...
  app.post("/api/v1/projects/:id/custom-tools", async (req) => {
    ...
    if (count >= CUSTOM_TOOL_LIMITS.maxPerProject) {
      throw new AppError(
        `Maximum ${CUSTOM_TOOL_LIMITS.maxPerProject} custom tools per project`,
        403,
        "LIMIT_EXCEEDED",
      );
    }
  ...
  app.patch("/api/v1/projects/:id/custom-tools/:tid", async (req) => {
    ...
    if (!existing) throw new NotFoundError("Custom tool not found");
...
export async function loadEnabledCustomTools(projectId: string) {
  const rows = await prisma.customTool.findMany({
