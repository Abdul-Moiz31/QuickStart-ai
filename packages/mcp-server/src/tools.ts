...
import type { QuickStartApiClient } from "./api-client.js";
...
export function registerQuickStartTools(server: McpServer, getClient: () => QuickStartApiClient) {
  server.tool(
    ...
    "Get the QuickStart AI business profile for the connected account (name, industry, description, location, support email, onboarding Q&A).",
    {},
    async () => {
  ...
  server.tool(
    ...
    "Save or update business details in QuickStart AI. Provide any fields to change; omitted fields stay as-is. Updates the chatbot knowledge automatically.",
    {
  ...
  server.tool(
    "list_faqs",
    "List FAQ knowledge entries for the QuickStart AI project (question/answer pairs the chatbot uses).",
    {
  ...
  server.tool(
    "add_faq",
    "Add a FAQ question and answer to the QuickStart AI chatbot knowledge base.",
    {
  ...
  server.tool(
    "list_projects",
    "List QuickStart AI projects for the connected account (useful to find projectId).",
    {},
    async () => {
      const client = getClient();
  ...
  server.tool(
    "list_conversations",
    "List recent visitor chat conversations for a QuickStart AI project (excludes admin test sessions).",
    {
      projectId: z.string().uuid().optional().describe("Project ID; defaults to your primary project"),
      limit: z
        .number()
  ...
  server.tool(
    "get_conversation",
    "Get a full visitor conversation with all messages by session ID.",
    {
  ...
  server.tool(
    ...
    "Search visitor conversations by visitor name, email, or message content.",
    {
