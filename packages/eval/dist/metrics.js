...
export function scoreFaithfulness(answer, contexts) {
    if (!contexts.length)
        return answer.length === 0 ? 1 : 0.2;
    const ctxTokens = tokenize(contexts.join(" "));
    const ansTokens = tokenize(answer);
    return jaccard(ansTokens, ctxTokens);
}
export function scoreAnswerRelevancy(question, answer) {
    return jaccard(tokenize(question), tokenize(answer));
}
export function scoreContextRelevance(question, contexts) {
    if (!contexts.length)
        return 0;
    return jaccard(tokenize(question), tokenize(contexts.join(" ")));
}
export function scoreExpectedOverlap(answer, expected) {
    return jaccard(tokenize(answer), tokenize(expected));
}
export async function runEval(cases, answerFn) {
    const results = [];
    ...
    const avg = (key) => results.reduce((s, r) => s + r[key], 0) / (results.length || 1);
    return {
...
export const SAMPLE_GOLDEN_SET = [
    {
        id: "hours",
...
export function casePassed(r) {
    if (r.expectedOverlap >= 0.16)
        return true;
    return r.faithfulness >= 0.2 && r.answerRelevancy >= 0.1;
}
export function evaluatePass(summary, thresholds) {
    ...
    if (summary.faithfulness < thresholds.faithfulness) {
        reasons.push(`Faithfulness ${(summary.faithfulness * 100).toFixed(0)}% below ${(thresholds.faithfulness * 100).toFixed(0)}%`);
    }
    if (summary.answerRelevancy < thresholds.answerRelevancy) {
        reasons.push(`Answer relevancy ${(summary.answerRelevancy * 100).toFixed(0)}% below ${(thresholds.answerRelevancy * 100).toFixed(0)}%`);
    }
    if (summary.expectedOverlap < thresholds.expectedOverlap) {
        reasons.push(`Expected overlap ${(summary.expectedOverlap * 100).toFixed(0)}% below ${(thresholds.expectedOverlap * 100).toFixed(0)}%`);
    }
    if (passRate < thresholds.minPassRate) {
        reasons.push(`Only ${(passRate * 100).toFixed(0)}% of questions passed (need ${(thresholds.minPassRate * 100).toFixed(0)}%)`);
    }
    return {
