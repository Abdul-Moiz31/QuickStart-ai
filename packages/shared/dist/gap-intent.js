 ...
 */
export function isGapExcludedUserMessage(message) {
    const trimmed = message.trim();
    if (!trimmed)
        return true;
    if (trimmed.length <= MAX_GREETING_LEN && GREETING_PATTERN.test(trimmed))
        return true;
    if (trimmed.length <= 40 && ACK_PATTERN.test(trimmed))
        return true;
...
export function isHandoffAssistantTurn(meta) {
 ...
 */
export function shouldExcludeFromGaps(userMessage, assistantMeta) {
    if (isGapExcludedUserMessage(userMessage))
        return true;
    if (isHandoffAssistantTurn(assistantMeta))
        return true;
    return false;
}
//# sourceMappingURL=gap-intent.js.map