...
function categoryStatus(score: number, threshold: number, pending: boolean): EvalCategory["status"] {
  if (pending) return "pending";
  if (score >= threshold) return "good";
  if (score >= threshold * 0.55) return "fair";
  return "weak";
}

/** Faster responses score higher (3s ≈ 100%, 20s+ ≈ low). */
export function latencyScore(avgLatencyMs: number): number {
  if (avgLatencyMs <= 0) return 0;
  if (avgLatencyMs <= 3000) return 1;
  if (avgLatencyMs >= 20000) return 0.12;
...
export function readinessLabel(score: number, hasRun: boolean) {
  if (!hasRun) return "Run eval to score quality";
  if (score >= 0.75) return "Strong for production";
  if (score >= 0.5) return "Usable — room to improve";
  if (score > 0) return "Needs work in some areas";
