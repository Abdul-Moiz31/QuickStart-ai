...
import { hybridRetrieve, rerankChunks } from "./retrieve.js";
...
export function buildAgentTools(ctx) {
    const tools = [
        {
            ...
            async execute() {
                if (!ctx.chunks.length)
                    return { output: "No knowledge snippets found." };
                return {
                    output: ctx.chunks
        ...
        },
        {
            ...
            async execute() {
                return {
                    output: ctx.businessHours ?? "Business hours are not configured. Suggest contacting support.",
                };
            },
        },
    ];
    if (ctx.toolsWebSearch) {
        tools.push({
            name: "web_search",
            description: "Fetch public web context from the business website when knowledge base is insufficient",
            async execute(args) {
    ...
    if (ctx.toolsHumanHandoff !== false) {
        tools.push({
            ...
            description: "Signal that a human agent should follow up when the visitor asks for a person or the bot cannot help",
            async execute(args) {
    ...
    if (ctx.toolsLeadCapture) {
        tools.push({
            name: "capture_lead",
            description: "Record visitor contact details when they want pricing, a demo, or a callback. Requires name and email.",
            async execute(args) {
...
export function buildAgentSystemPrompt(projectName, custom) {
...
async function runToolLoopPrelude(opts) {
    const toolsUsed = ["search_knowledge"];
    const eventsEmitted = [];
    ...
    try {
        ...
        if (match) {
            ...
            selectedTools = (parsed.tools ?? []).filter((n) => n !== "search_knowledge" && opts.tools.some((t) => t.name === n));
    ...
    const toolResults = [`search_knowledge:\n${opts.knowledge}`];
    for (const name of selectedTools.slice(0, 2)) {
        ...
        toolsUsed.push(name);
        if (result.event)
            eventsEmitted.push(result.event);
    }
 ...
 */
export async function* runAgenticRagStream(opts) {
    ...
    const knowledge = knowledgeResult.output;
    const confidence = computeConfidence(chunks);
    const preEventsEmitted = [];
    if (confidence === "low" && chunks.length === 0) {
        const escalate = tools.find((t) => t.name === "escalate_to_human");
        if (escalate) {
            const esc = await escalate.execute({ reason: "insufficient knowledge base coverage" });
            if (esc.event)
                preEventsEmitted.push(esc.event);
        }
    }
    const system = buildAgentSystemPrompt(opts.projectName, opts.systemPrompt);
