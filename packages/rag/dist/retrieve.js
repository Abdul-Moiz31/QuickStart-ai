...
export async function vectorSearch(projectId, embedding, topK = DEFAULT_TOP_K) {
...
export async function keywordSearch(projectId, query, topK = DEFAULT_TOP_K) {
    const rows = await prisma.$queryRawUnsafe(`
    SELECT id, content, "documentId",
...
export async function hydeRetrieve(opts) {
    ...
    try {
        const hypo = await chat.chat([
            {
                role: "system",
                content: "Write a short hypothetical passage that would answer the user question as if from a business knowledge base. No preamble.",
            },
...
export async function rerankChunks(query, chunks, chat, topK = 8) {
    if (chunks.length <= 1)
    ...
    try {
        const raw = await chat.chat([
            {
                role: "system",
                content: "Score each passage 0-10 for relevance to the query. Reply JSON only: {\"scores\":[...]} with one score per passage index, same order.",
            },
...
export async function storeChunkEmbeddings(chunkIds, embeddings) {
    await Promise.all(chunkIds.map((id, i) => {
        const emb = embeddings[i];
        if (!id || !emb)
            return Promise.resolve();
        return prisma.$executeRawUnsafe(`UPDATE "KnowledgeChunk" SET embedding = $1::vector WHERE id = $2::uuid`, toVectorLiteral(emb), id);
    }));
}
